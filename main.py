...
def helper(dis):
    ...
    pre = [col for col in pre.values[0]]  # Flatten the list

    # Fetch medications
    med = medications[medications['Disease'] == dis]['Medication']
    med = [m for m in med.values]  # Flatten the list

    # Fetch recommended diet
    die = diets[diets['Disease'] == dis]['Diet']
    die = [d for d in die.values]  # Flatten the list

    # Fetch workout recommendations
    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = [w for w in wrkout.values]  # Flatten the list
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:  # Check if the symptom is valid
            ...
            return None  # Return None if any symptom is invalid
...
def home():
    if request.method == "POST":
        ...
        print("User Input Symptoms:", symptoms)  # Debugging

        if not symptoms:
            message = "Please enter symptoms or check for misspelled symptoms."
            return render_template("EnterSymptoms.html", message=message)

        # Split user input into a clean list of symptoms (strip spaces, convert to lowercase)
        user_symptoms = [s.strip().lower() for s in symptoms.split(",")]

        # Debugging step: Check cleaned symptoms
        print("Processed Symptoms List:", user_symptoms)

        # Validate symptoms (checking lowercase keys in the dictionary)
        invalid_symptoms = [symptom for symptom in user_symptoms if symptom not in symptoms_dict.keys()]
        if invalid_symptoms:
            message = f"Invalid or misspelled symptoms: {', '.join(invalid_symptoms)}. Please check and try again."
            return render_template("EnterSymptoms.html", message=message)
        ...
        if not predicted_disease:
            message = "Unable to predict due to invalid symptoms. Please check and try again."
            return render_template("EnterSymptoms.html", message=message)
        ...
        my_precautions = list(precautions) if precautions else []
        my_medications = list(medications) if medications else []
        my_diet = list(rec_diet) if rec_diet else []
        my_workout = list(wrkout) if wrkout else []
